...
let filikaService = null;
// Tool 1: Maaş kodlarını getir
mcpServer.tool("filika_get-wage-codes", "Filika.co sistemindeki maaş kodlarını getirir", {
...
mcpServer.tool("filika_calculate-payroll", "Verilen parametrelerle bordro hesaplar", {
...
mcpServer.tool("filika_calculate-severance", "Kıdem tazminatı hesaplar", {
...
mcpServer.tool("filika_get-employee-list", "Çalışanlarla ilgili detaylı liste getirir", {
    ...
    try {
        const result = await filikaService.getEmployeeList(args.month);
        return {
            content: [
                {
                    type: "text",
...
export function setFilikaToken(token) {
